let peliculas = [
  ...
  //funcion que agrega el atributo generos que es un arreglo el cual contiene
  ...
  const agregarGeneros = () => {
      peliculas = peliculas.map((peli) => {
          ...
          peli.genre_ids.forEach((id) => {
              ...
              let objetoGeneroEncontrado = generos.find((gen) =>{
                  if(gen.id === id){
                      return gen
                  }

              })
              generosString.push(objetoGeneroEncontrado.name)
          }
          )
